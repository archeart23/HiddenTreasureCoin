...
// Copyright (c) 2018 The HiddenTreasure developers
...
#if defined(NDEBUG)
# error "Hiddentreasurecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfc1df07cee353a420603bf2e99763e39128ebbc8975d6411bdbe8ff20338a0a8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hiddentreasurecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hiddentreasurecoin Signed Message:\n";
...
{
    // Hiddentreasurecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 750000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 750000); // Hiddentreasurecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Hiddentreasurecoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Hiddentreasurecoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hiddentreasurecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hiddentreasurecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1d32e40f55f1d12d8a51ba20f3725a93613f4150e26c795595af96d69293a6a9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "A day after the Queen's Birthday";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f6da509fb6243f3519a80e0f6205cc852d56d1c32a01540809487d316f0ef0989dca183f3ca90210dd4f6466a2f4eb68d416a8f75f4a490885836e2d83ce4ae1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528766670;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084533777;

        if (fTestNet)
        {
            block.nTime    = 1528766658;
            block.nNonce   = 385402408;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe1931b0bdd91706e5a20f834347e02a2225c80a0555aaf3102779b8f7e7dc50e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xc8, 0xb7, 0xd6 }; // Hiddentreasurecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("HiddentreasurecoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HiddentreasurecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HiddentreasurecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HiddentreasurecoinMiner(CWallet *pwallet)
{
    printf("HiddentreasurecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hiddentreasurecoin-miner");
    ...
    try { loop {
        ...
        printf("Running HiddentreasurecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HiddentreasurecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HiddentreasurecoinMiner, pwallet));
}
